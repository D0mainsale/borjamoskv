...
# =====================================================================
# CORTEX VIBE AGENT (Núcleo Soberano v1.0)
...
SYSTEM_PROMPT = """
Eres CORTEX, un agente editor de código 100% autónomo.
...
def parse_and_apply_edits(response_text: str):
    ...
    if not edits:
        print("[CORTEX WARNING] No se detectaron vectores XML de edición. Posible alucinación.")
...
def run_agent(prompt: str, files: List[str]):
    print(f"[CORTEX] Extrayendo O(1) memoria de {len(files)} nodos...")
    ...
    print("[CORTEX] Enviando flujo de carga a Ollama Local ($0 Coste)...")
    ...
    try:
        ...
        print(f"[ERROR CORTEX] Falla estructural: {e}")

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("\n=== CORTEX VIBE AGENT ===")
        print("Uso Soberano:")
        print("  python cortex_vibe_agent.py \"Instrucción en Lenguaje Natural\" archivoA.js archivoB.html\n")
        sys.exit(1)
