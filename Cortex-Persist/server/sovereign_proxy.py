...
# ── Sovereign Substrate ─────────────────────────────────────────────────────
from server.legion_orchestrator import LEGION_COMMANDER
from server.vsa_engine import MemoryConsolidator
...
MEMORY_CONSOLIDATOR = MemoryConsolidator(
    os.path.join(BASE_PATH, "sovereign_memory.bin")
)
SOVEREIGN_FORENSIC_LEDGER = os.path.join(BASE_PATH, "forensic_ledger.jsonl")
...
ARCHI_ROOT = "/Users/borjafernandezangulo/10_PROJECTS/agents-archi"
if ARCHI_ROOT not in sys.path:
    sys.path.append(ARCHI_ROOT)

# Agentic Runtime Bridge
try:
    from nucleo_soberano.motor import MotorSoberano as CortexRuntime
except ImportError:
    CortexRuntime = None

# Ω-PERSIST: Write-path verification membrane
try:
    from nucleo_soberano.membrana import guard_and_commit_sync
...
try:
    from nucleo_soberano.memoria import AlmacenMemoria
...
try:
    from nucleo_soberano.gobernador_homeostasis import GobernadorDimensional
...
class ReconWorker(threading.Thread):
    ...
    def check_ledger_pulse(self) -> Optional[dict]:
        ledger_path = os.path.join(self.base_path, "swarm_ledger.jsonl")
...
class SovereignRequestHandler(BaseHTTPRequestHandler):
        ...
        if self.path == "/health" or self.path == "/api/health":
            ...
            self.wfile.write(
                json.dumps(
                    {
                        "steps": [
                            "- [x] Iniciar sustrato `nucleo_soberano`",
    ...
    def do_POST(self):
        ...
        if self.path == "/api/ouroboros/extract":
            ...
            base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            script_path = "/Users/borjafernandezangulo/10_PROJECTS/agents-archi/scripts/semantic_time_machine.py"
            log_file = os.path.join(base_dir, "server", "data", "temporal_index.log")
            try:
                # Disparo asíncrono
