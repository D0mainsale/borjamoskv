#!/usr/bin/env python3
"""
cortex.tools.bounty_scout.py — Caza-Recompensas Scout Engine v1.0
──────────────────────────────────────────────────────
...
Output:
  ...
  - cortex/data/bounties.jsonl (ledger de programas descubiertos)
...
BOUNTIES_DB  = Path("cortex/data/bounties.jsonl")
...
def save_bounties_ledger(programs: list[BountyProgram]) -> None:
    """Persist discovered programs to cortex/data/bounties.jsonl."""
...
def scout(min_reward: int = MIN_REWARD, max_count: int = MAX_BOUNTIES, auto_enqueue: bool = False) -> Dict[str, Any]:
    """
    Programmatic entry point for the CORTEX Worker.
    """
    global MIN_REWARD, MAX_BOUNTIES
    MIN_REWARD = min_reward
    MAX_BOUNTIES = max_count
    ...
    if programs:
        ...
        if auto_enqueue:
            try:
                ...
                print(f"{_GRN}  ✓ {len(programs)} tasks enqueued in IndustrialQueue{_R}")
            except ImportError:
                print(f"{_RED}  ⚠ IndustrialQueue not available for auto-enqueue{_R}")

    return {
        "status": "complete" if programs else "no_results",
        "count": len(programs),
