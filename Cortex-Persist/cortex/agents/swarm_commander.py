#!/usr/bin/env python3
"""
cortex.agents.swarm_commander.py — Caza-Recompensas Swarm Commander v1.0
────────────────────────────────────────────────────────────
Orquestador del swarm completo:

  FASE 1 — SCOUT:   cortex.tools.bounty_scout.py → tasks.md
  FASE 2 — HUNT:    cortex.agents.ralph_parallel.py → workers claim tasks
  FASE 3 — REPORT:  cortex.tools.bounty_hunter.py por cada worker
  FASE 4 — SUBMIT:  capital_extractor.py (C5-REAL si credenciales)

Uso rápido:
  python cortex.agents.swarm_commander.py                  # full cycle
  python cortex.agents.swarm_commander.py --phase scout    # solo discovery
  python cortex.agents.swarm_commander.py --phase hunt     # solo analysis
  python cortex.agents.swarm_commander.py --phase status   # dashboard
...
BOUNTIES_DB   = BASE_DIR / "cortex/data/bounties.jsonl"
FINDINGS_LOG  = BASE_DIR / "cortex/data/findings.jsonl"
REPORTS_DIR   = BASE_DIR / "bounty_reports"
LEDGER_FILE   = BASE_DIR / "cortex/data/swarm_ledger.jsonl"
...
def phase_scout() -> int:
    """Run cortex.tools.bounty_scout.py to populate tasks.md."""
    _log("PHASE 1 — SCOUT: Discovering bounties...", _BLU)

    script = BASE_DIR / "cortex.tools.bounty_scout.py"
...
def _hunt_worker(worker_id: int, task_label: str) -> dict:
    ...
    # Dynamic import of cortex.tools.bounty_hunter
    try:
...
def phase_submit() -> None:
    ...
    if not FINDINGS_LOG.exists():
        _log("No cortex/data/findings.jsonl — nothing to submit", _YEL)
        return
...
def phase_status() -> None:
    ...
    if FINDINGS_LOG.exists():
        ...
        print(f"  Findings  : {_DIM}No cortex/data/findings.jsonl{_R}")
