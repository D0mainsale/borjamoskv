#!/usr/bin/env python3
"""
cortex.agents.ralph_parallel.py — Ralph Wickham · Parallel Worker Pool v2.0
──────────────────────────────────────────────────────────────
Cada worker:
...
Mecanismo de claim:
  - fcntl.flock (POSIX advisory lock) sobre `.cortex.agents.ralph.lock`
...
LEDGER_FILE    = Path("cortex.agents.cortex/data/ralph_ledger.jsonl")
LOCK_FILE      = Path(".cortex.agents.ralph.lock")
LOG_DIR        = Path("logs")
...
def run() -> None:
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS, thread_name_prefix="cortex.agents.ralph") as pool:
        futures = {
...
if __name__ == "__main__":
    # Allow: python cortex.agents.ralph_parallel.py [--workers N]
    import argparse
